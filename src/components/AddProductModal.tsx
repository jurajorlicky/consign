...
export default function AddProductModal({ isOpen, onClose, onProductAdded }: AddProductModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      getFees().then(adminFees => {
        ...
        setFees({ fee_percent: 0.2, fee_fixed: 5 });
      });
    } else {
    ...
    const debounceTimeout = setTimeout(async () => {
      ...
      try {
        ...
        if (error) {
          ...
          setError('Chyba pri vyhľadávaní produktov');
          setExistingProducts([]);
        } else {
  ...
  const handleProductSelect = async (product: Product) => {
    ...
    try {
      ...
      if (error) {
        ...
        setError('Chyba pri načítavaní veľkostí');
        setSizes([]);
        return;
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'Chyba pri pridávaní produktu');
    } finally {
      setLoading(false);
    }
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            {!selectedProduct ? (
              <div className="space-y-4">
                ...
                {/* No results message */}
                {searchTerm.length >= 2 && !searchLoading && existingProducts.length === 0 && (
                  <div className="text-center py-8 text-gray-500">
                    Žiadne produkty nenájdené pre "{searchTerm}"
                  </div>
                )}
              </div>
            ) : (
